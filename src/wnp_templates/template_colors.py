...
_FAMILIES_YAML: pathlib.Path = pathlib.Path(__file__).parent / "families.yaml"


def _load_families() -> tuple[dict[str, dict[str, str]], dict[str, str]]:
    """Return (family → {effect_label: stem}, family → description) from families.yaml.

    families.yaml is generated by tools/build_templates.py from the
    label/effect_label fields in template-src/configs/ — the single source
    of truth for the display-family registry.
    """
    if not _FAMILIES_YAML.exists():
        logging.warning("families.yaml missing from %s", _FAMILIES_YAML)
        return {}, {}
    try:
...
# The first entry in each family is the canonical variant.
...
TEMPLATE_FAMILIES, TEMPLATE_FAMILY_DESCRIPTIONS = _load_families()
